...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex">
      ...
      {/* ── SIDEBAR ───────────────────────────────────────── */}
      <aside style={{width:'232px'}} className="fixed h-full flex flex-col bg-white border-r border-slate-100 z-40">
        ...
        <div className="px-5 h-16 flex items-center border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-base font-700 text-slate-900 tracking-tight">Settle</span>
          </div>
        </div>
        ...
        <div className="px-4 pt-4 pb-2">
          <Link href="/invoices/new"
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-600 text-white transition-all duration-150 active:scale-[0.98]"
        ...
        {profile?.subscription_tier !== 'pro' && (
          <div className="mx-3 mb-3">
            <Link href="/settings?tab=billing"
              ...
              style={{background:'linear-gradient(135deg, #0f0d38 0%, #1e1b6e 100%)'}}>
              <p className="text-xs font-600 text-amber-400 mb-0.5">Upgrade to Pro</p>
        ...
        <div className="px-3 pb-4 pt-3 border-t border-slate-100">
          <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-slate-50 transition-colors mb-1">
            <div className="w-8 h-8 rounded-full flex items-center justify-center font-700 text-amber-700 text-sm flex-shrink-0"
              style={{background:'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)'}}>
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-600 text-slate-800 truncate">{profile?.business_name ?? profile?.full_name}</p>
      ...
      {/* ── MAIN ──────────────────────────────────────────── */}
      <main className="flex-1 min-h-screen p-8" style={{marginLeft:'232px'}}>
        {children}
      </main>
    </div>
  )
