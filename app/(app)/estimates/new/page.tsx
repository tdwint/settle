...
export default function NewEstimatePage() {
  ...
  async function handleSubmit(e: React.FormEvent, action: 'draft' | 'send') {
    ...
    if (action === 'send') {
      await fetch(`/api/estimates/${data.data.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-800 text-gray-900">New estimate</h1>
          <p className="text-gray-500 mt-1">Estimates are unlimited on all plans.</p>
        </div>
        <Link href="/estimates" className="btn-secondary text-sm">Cancel</Link>
      </div>
      ...
      <form onSubmit={(e) => handleSubmit(e, 'draft')} className="space-y-6">
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input className="input" value={clientAddress} onChange={e => setClientAddress(e.target.value)} placeholder="123 Main St, City, State" />
            </div>
            <div className="col-span-2 md:col-span-1">
        ...
        {/* DETAILS */}
        <div className="card p-6">
          <h2 className="font-700 text-gray-900 mb-4">Estimate details</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-600 text-gray-700 mb-1.5">Valid until</label>
              <input className="input" type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} />
              <p className="text-xs text-gray-400 mt-1">Client sees this date on the estimate</p>
            </div>
            <div>
        ...
        <div className="card p-6">
          ...
          <textarea className="input resize-none" rows={3} value={notes} onChange={e => setNotes(e.target.value)}
            placeholder="Scope notes, terms, or anything else for the client…" />
        </div>
